...
\begin{code}
-- size is only defined for List, not ListN or ListX, so that is why you get unbound symbol error
-- at first
...
replicate  n a = add a (replicate (n-1) a)
\end{code}
...
\begin{code}
{-@ map :: (a -> b) -> lst:List a -> ListN b {size lst} @-}
map f Emp        = Emp
...
\begin{code}
{-@ zipWith :: (a -> b -> c) -> l1:List a -> ListN b {size l1} -> ListN c {size l1} @-}
...
{-@ measure allSize      :: List (List a) -> Int
    allSize (Emp)        = 0
    allSize ((:+:) x xs) = (size x) + allSize xs
@-}
...
{-@ concat' :: l1:List a -> l2:List a -> ListN a {size l1 + size l2}  @-}
concat' Emp Emp = Emp
concat' Emp l2 = l2
concat' l1 Emp = l1
concat' (x :+: xs) l2 = concat' xs  (x :+: l2)




{-@ concat :: lst:List (List a) -> {v:List a | size(v) == allSize(lst)}  @-}
concat Emp = Emp
