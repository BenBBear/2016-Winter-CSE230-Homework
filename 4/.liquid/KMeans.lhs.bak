...
{-@ type PointNE = {v:Point | 0 < size(v)} @-}



{-@ measure clusterDimension :: Cluster -> Int
    clusterDimension (a , p) = size(p)
@-}
...
\begin{code}
{-@ distance :: n:Pos -> PointN n -> PointN n -> Double @-}
distance n px py = sqrt $ foldr (+) 0 $ zipWith (\x y -> abs (x^2 - y^2)) px py
\end{code}
...
\begin{code}
{-@ nearest :: k:Pos -> n:Pos -> CenteringKN k n -> PointN n -> CenterK k @-}
