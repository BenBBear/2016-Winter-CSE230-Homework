...
evalS (Throw e)     =  do
              v     <- evalE e
              throwError v
evalS (Try s1 x s2)   =  do
               catchError (evalS s1) (\e -> do
                             store <- get
                             put (Map.insert x e store)
                             evalS s2
                          )
...
-- ((value, consoleaLog), store)
runEval act sto =  runState (runWriterT (runErrorT  act)) sto

  -- runState (runWriter (runErrorT act)) sto
...
execute sto stmt   =   (rsto, mbval, str)
                        where
                            ((val, str), rsto) = runEval (evalS stmt) sto
                            mbval = case val of
                                    Right _ -> Nothing
                                    Left  x -> Just x
...
bst2List :: BST k v -> [(k,v)]
bst2List Emp = []
bst2List (Bind kt vt ltree rtree) =  bst2List ltree ++ [(kt,vt)] ++ bst2List rtree
splitHalf l = splitAt ((length l + 1) `div` 2) l


list2Bst :: [(k,v)] -> BST k v
list2Bst [] = Emp
list2Bst lst =  Bind kc vc (list2Bst l1) (list2Bst l2)
                where
                    (l,l2) = splitHalf lst
                    (kc,vc) = last l
                    l1 = take (length l -1) l
-- take (length  [1,2,3]  -1)  [1,2,3]


bstBalance :: BST k v -> BST k v
bstBalance t = list2Bst ( bst2List t )
...
bstInsert' k v (Bind kt vt ltree rtree)
                                      | kt == k = (Bind kt v ltree rtree)
                                      | k > kt = (Bind kt vt ltree (bstInsert' k v rtree))
                                      | k < kt = (Bind kt vt (bstInsert' k v ltree) rtree)

bstInsert :: (Ord k) => k -> v -> BST k v -> BST k v
bstInsert k v tree = bstBalance (bstInsert' k v tree)
...
bstDelete' _ Emp = Emp
bstDelete' k (Bind k' _ Emp r) | k == k' = r
bstDelete' k (Bind k' _ l Emp) | k == k' = l
...
bstDelete :: (Ord k) => k -> BST k v -> BST k v
bstDelete k tree = bstBalance (bstDelete' k tree)
...
genBal'' :: Int -> Int -> BST Int Char
genBal'' 0 _ = Emp
genBal'' h k = Bind k '0' (genBal'' (h-1) (k-1)) (genBal'' (h-1) (k+1))


genBal' :: Int -> BST Int Char
genBal' h =  genBal'' h 0

genBal :: Gen (BST Int Char)
genBal = liftM genBal' (elements [1..10])
...
prop_bitSubtractor_Correct bin xs =
  binary (sampleN out) == if (b1>=b2) then b1-b2 else 0
    where b1          = binary xs
          b2          = binary (sample1 bin)
          (out, bout) = bitSubtractor (bin, map lift0 xs)
...
bitSubtractor (bin, x:xs) = (muxN (repeat low, diff:diffs, bout) , low)
...
-- bitSub  (high,[low,high,high]) | 110 - 1

-- Problem: Multiplication
-- -----------------------

bitMulAdd :: (Signal, Signal, Signal, Signal) -> (Signal, Signal)
bitMulAdd (x, y, sumin, cin)  = (sumout, cout)
      where (sumout, cout) = fulladd (cin, sumin, y2)
            y2             = and2 (x, y)
...
prop_Multiplier_Correct  b1 b2 = binary (sampleN p) == (binary b1) * (binary b2)
                                 where p = multiplier (map lift0 b1, map lift0 b2)
...
multiplier (_,[])   = []
multiplier ([],_)   = []
multiplier (xs, bs) = multiplier' (xs, bs, repeat low, repeat low)

multiplier' :: ([Signal], [Signal], [Signal], [Signal]) -> [Signal]
multiplier' (xs,[],sumin,cin) = adder (sumin, cin)
multiplier' (xs,b:bs,sin,cin) = [head souts] ++ (multiplier' (xs, bs, (tail souts) ++ [low], couts))
    where (souts,couts) = bitMultiplier (xs,b,sin,cin)
...
bitMultiplier ([],_,_,_) = ([],[])
bitMultiplier (x:xs, y, s:sumins, c:cins) = (sumout:sumouts, cout:couts)
                    where (sumout,cout)   = bitMulAdd (x,y,s,c)
                          (sumouts,couts) = bitMultiplier (xs,y,sumins,cins)
